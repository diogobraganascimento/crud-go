...
func TesUserRepository_FindUserByEmail(t *testing.T) {
	databaseName := "user_database_test"
	collectionName := "user_collection_test"

	err := os.Setenv("MONFODB_USER_DB", collectionName)
	if err != nil {
		t.FailNow()
		return
	}
	...
	// Testing find by email not find error
...
func TesUserRepository_FindUserByEmailAndPassword(t *testing.T) {
	databaseName := "user_database_test"
	collectionName := "user_collection_test"
...
func TesUserRepository_FindUserById(t *testing.T) {
	databaseName := "user_database_test"
	collectionName := "user_collection_test"
	...
	// Testing find by id not find error
	mtestDb.Run("returns_no_document_found", func(mt *mtest.T) {
		mt.AddMockResponses(mtest.CreateCursorResponse(
			0,
			fmt.Sprintf("%s.%s", databaseName, collectionName),
		...
		assert.NotNil(t, err)
		assert.Equal(t, err.Message, fmt.Sprint("User not found whit this ID: test"))
		assert.Nil(t, userDomain)
	})
}
